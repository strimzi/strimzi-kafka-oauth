...
public class KeycloakWithJwtValidationAuthzOverPlainTest {
    ...
    Producer<String, String> recycleProducer(final String name, boolean recycle) {
        ...
        if (TEAM_A_CLIENT.equals(name)) {
            Properties producerProps = buildProducerConfigOAuthBearer(tokens.get(name));
            teamAProducer = new KafkaProducer<>(producerProps);
            return teamAProducer;
        } else {
            Properties producerProps = buildProducerConfigPlain(tokens.get(name));
            teamBProducer = new KafkaProducer<>(producerProps);
            return teamBProducer;
        }
    }
    ...
    Consumer<String, String> newConsumer(final String name, String topic) {
        ...
        if (TEAM_A_CLIENT.equals(name)) {
            Properties consumerProps = buildConsumerConfigOAuthBearer(tokens.get(name));
            consumerProps.setProperty(ConsumerConfig.GROUP_ID_CONFIG, groupFor(topic));
            teamAConsumer = new KafkaConsumer<>(consumerProps);
            return teamAConsumer;
        } else {
            Properties consumerProps = buildConsumerConfigPlain(TEAM_B_CLIENT, TEAM_B_CLIENT_SECRET);
            consumerProps.setProperty(ConsumerConfig.GROUP_ID_CONFIG, groupFor(topic));
            teamBConsumer = new KafkaConsumer<>(consumerProps);
            return teamBConsumer;
        }
    }
    ...
    private void testClusterManager() throws Exception {
        ...
        Properties bobAdminProps = buildProducerConfigOAuthBearer(tokens.get(BOB));
        Producer<String, String> producer = new KafkaProducer<>(bobAdminProps);

        Properties consumerProps = buildConsumerConfigOAuthBearer(tokens.get(BOB));
    ...
    static HashMap<String, String> authenticateAllActors() throws IOException {
        ...
        tokens.put(TEAM_A_CLIENT, loginWithClientSecret(URI.create(TOKEN_ENDPOINT_URI), null, null,
                TEAM_A_CLIENT, TEAM_A_CLIENT_SECRET, true, null, null).token());
        tokens.put(TEAM_B_CLIENT, loginWithClientSecret(URI.create(TOKEN_ENDPOINT_URI), null, null,
                TEAM_B_CLIENT, TEAM_B_CLIENT_SECRET, true, null, null).token());
    ...
    static Properties buildProducerConfigOAuthBearer(String accessToken) {
    ...
    static Properties buildProducerConfigPlain(String accessToken) {
        Properties p = new Properties();
        p.setProperty("security.protocol", "SASL_PLAINTEXT");
    ...
    static Properties buildAdminClientConfig(String accessToken) {
        return buildProducerConfigOAuthBearer(accessToken);
    }

    static Properties buildConsumerConfigOAuthBearer(String accessToken) {
        Properties p = new Properties();
        p.setProperty("security.protocol", "SASL_PLAINTEXT");
        p.setProperty("sasl.mechanism", "OAUTHBEARER");
        p.setProperty("sasl.jaas.config", "org.apache.kafka.common.security.oauthbearer.OAuthBearerLoginModule required " +
    ...
    static Properties buildConsumerConfigPlain(String clientId, String secret) {
        Properties p = new Properties();
        p.setProperty("security.protocol", "SASL_PLAINTEXT");
        p.setProperty("sasl.mechanism", "PLAIN");
        p.setProperty("sasl.jaas.config", "org.apache.kafka.common.security.plain.PlainLoginModule required " +
