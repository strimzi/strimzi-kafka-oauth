...
Let's make sure the SNAPSHOT Strimzi OAuth libraries are included.
    ...
    docker run --rm -ti $DOCKER_REG/strimzi/kafka:0.49.1-kafka-4.1.1 /bin/sh -c 'ls -la /opt/kafka/libs/kafka-oauth*'
...
You can explore the Kafka container more by starting it in interactive mode:
    ...
    docker run --rm -ti $DOCKER_REG/strimzi/kafka:0.49.1-kafka-4.1.1 /bin/sh
