...
This is a build of a Docker image based on `quay.io/strimzi/kafka:0.49.1-kafka-4.1.1` with added most recently locally built SNAPSHOT version of Strimzi Kafka OAuth libraries.
...
The result is that the specific version of Strimzi Kafka OAuth jars and their dependencies is used, because they appear on the classpath before the ones that are part of `quay.io/strimzi/kafka:0.49.1-kafka-4.1.1` which are located in the `/opt/kafka/libs` directory.
...
Run `mvn install` then, use `docker build` to build the image:
    ...
    docker build --output type=docker -t strimzi/kafka:latest-kafka-4.1.1-oauth .
...
For example, if you want to base your image on Strimzi Kafka 4.0.0 use `FROM quay.io/strimzi/kafka:0.49.1-kafka-4.0.0`.
...
You can start an interactive shell container and confirm that the jars are there.
    ...
    docker run --rm -ti strimzi/kafka:latest-kafka-4.1.1-oauth /bin/sh
    ls -la libs/oauth/
    echo "$CLASSPATH"
...
You achieve that by running the docker session as `root` user:
    ...
    docker run --rm -ti --user root strimzi/kafka:latest-kafka-4.1.1-oauth /bin/sh
...
You need to retag the built image, so you can push it to Docker Registry:
    ...
    docker tag strimzi/kafka:latest-kafka-4.1.1-oauth $DOCKER_REG/strimzi/kafka:latest-kafka-4.1.1-oauth
    docker push $DOCKER_REG/strimzi/kafka:latest-kafka-4.1.1-oauth

Actually, Kubernetes Kind supports an even simpler option how to make an image available to Kubernetes:

    kind load docker-image strimzi/kafka:latest-kafka-4.1.1-oauth 
...
This image builds the kafka-4.1.1 replacement image, so we need to replace all occurrences where kafka-4.1.1 is referred to into the proper coordinates to our image:

    sed -Ei "s#quay.io/strimzi/kafka:latest-kafka-4.1.1#${DOCKER_REG}/strimzi/kafka:latest-kafka-4.1.1-oauth#" \
...
You can also run the operator via its Helm chart and set the `kafka.image.registry` property to your local registry. As an example, if you've built and tagged the image as `local.dev/strimzi/kafka:0.49.1-kafka-4.1.1`. You can run it using the operator as:
