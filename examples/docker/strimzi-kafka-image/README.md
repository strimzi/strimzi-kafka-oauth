...
This is a build of a Docker image based on `quay.io/strimzi/kafka:0.23.0-kafka-2.8.0` with added most recently locally built SNAPSHOT version of Strimzi Kafka OAuth libraries.
...
The result is that the most recent Strimzi Kafka OAuth jars and their dependencies are used, because they appear on the classpath before the ones that are part of `quay.io/strimzi/kafka:0.23.0-kafka-2.8.0` which are located in the `/opt/kafka/libs` directory.
